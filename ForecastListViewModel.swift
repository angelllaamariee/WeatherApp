            ...
            CLGeocoder().geocodeAddressString(location) { (placemarks, error) in
                ...
                if let lat = placemarks?.first?.location?.coordinate.latitude,
                   let lon = placemarks?.first?.location?.coordinate.longitude {
                            apiService.getJSON(urlString: "https://api.openweathermap.org/data/2.5/onecall?lat=40.850518&lon=-74.043693&exclude=current,minutely,hourly,&appid=3d98f1fbb5044150153de4a70901eed4",
